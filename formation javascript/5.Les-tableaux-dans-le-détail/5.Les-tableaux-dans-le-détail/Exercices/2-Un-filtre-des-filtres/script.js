...
const filteredCompanies = companies.filter(companie => companie.revenue > 500);
...
const truthyValues = values.filter(vrai => vrai);
...
for(i = 0; i < names.length; i++){
  let filtre = names[i].split("");

    if(filtre[0] === "A" && filtre.length >= 5)
    {
      filtre.join("");
      filteredNames.push(filtre);
    }
}
